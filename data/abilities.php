<?php

return [
    'admins' => [
        'name' => t('dashboard.Admins', 'Admins'),
        'view' => t('dashboard.View_Admins', 'View Admins'),
        'create' => t('dashboard.Create_Admin', 'Create Admin'),
        'edit' => t('dashboard.Edit_Admin', 'Edit Admin'),
        'delete' => t('dashboard.Delete_Admin', 'Delete Admin'),
        'super' => t('dashboard.Super_Admin', 'Super Admin'),
    ],
    'languages' => [
        'name' => t('dashboard.Languages', 'Languages'),
        'view' => t('dashboard.View_Languages', 'View Languages'),
        'create' => t('dashboard.Create_Language', 'Create Language'),
        'edit' => t('dashboard.Edit_Language', 'Edit Language'),
        'delete' => t('dashboard.Delete_Language', 'Delete Language'),
    ],
    'translation-values' => [
        'name' => t('dashboard.Translation_Values', 'Translation Values'),
        'view' => t('dashboard.View_Translation_Values', 'View Translation Values'),
        'create' => t('dashboard.Create_Translation_Value', 'Create Translation Value'),
        'edit' => t('dashboard.Edit_Translation_Value', 'Edit Translation Value'),
        'delete' => t('dashboard.Delete_Translation_Value', 'Delete Translation Value'),
    ],
];
