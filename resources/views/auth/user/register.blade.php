...
<head>
  ...
  <title>حساب مستخدم جديد | اسأل قلقيلية</title>
...
<body class="min-h-screen bg-[#F8FAFC] text-slate-900 antialiased [font-family:Tajawal,Inter,sans-serif]">
  <main class="mx-auto grid max-w-7xl gap-6 px-4 py-10 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:px-8 lg:py-16">
    <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
      <p class="inline-flex rounded-2xl bg-green-50 px-3 py-1.5 text-sm font-extrabold text-[#16A34A]">مستخدم جديد</p>
      <h1 class="mt-4 text-3xl font-extrabold text-slate-950">إنشاء حساب مستخدم</h1>
      <form class="mt-7 grid gap-5" action="{{ route('register.store') }}" method="post">
        ...
        <button type="submit" class="inline-flex h-14 items-center justify-center rounded-2xl bg-[#2563EB] px-6 text-base font-extrabold text-white">إنشاء الحساب</button>
      </form>
    ...
    <aside class="rounded-2xl border border-slate-200 bg-slate-950 p-6 text-white shadow-sm sm:p-8">
      <h2 class="text-3xl font-extrabold leading-tight">حساب واحد لخدمات المدينة</h2>
    </aside>
  </main>
</body>
</html>