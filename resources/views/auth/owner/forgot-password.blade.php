...
<head>
  ...
  <title>استعادة كلمة المرور لصاحب النشاط</title>
...
<body class="min-h-screen bg-[#F8FAFC] text-slate-900 antialiased [font-family:Tajawal,Inter,sans-serif]">
  <main class="mx-auto max-w-3xl px-4 py-12">
    <section class="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
      <h1 class="text-3xl font-extrabold">استعادة كلمة المرور</h1>
      <form class="mt-6 grid gap-5" action="{{ route('owner.password.email') }}" method="post">
        @csrf
        @if (session('status'))
        ...
        <button type="submit" class="inline-flex h-14 items-center justify-center rounded-2xl bg-[#2563EB] px-6 text-base font-extrabold text-white">إرسال رابط الاستعادة</button>
      </form>
    </section>
  </main>
</body>
