...
<head>
  ...
  <title>دخول صاحب النشاط | اسأل قلقيلية</title>
...
<body class="min-h-screen bg-[#F8FAFC] text-slate-900 antialiased [font-family:Tajawal,Inter,sans-serif]">
  <main class="mx-auto grid max-w-7xl gap-6 px-4 py-10 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:px-8 lg:py-16">
    <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
      <p class="inline-flex rounded-2xl bg-blue-50 px-3 py-1.5 text-sm font-extrabold text-[#2563EB]">حساب صاحب النشاط</p>
      <h1 class="mt-4 text-3xl font-extrabold text-slate-950">دخول صاحب النشاط</h1>
      <p class="mt-3 text-base leading-7 text-slate-600">ادخل بالبريد الإلكتروني أو رقم الهاتف لإدارة نشاطك.</p>
      <form class="mt-7 grid gap-5" action="{{ route('owner.login.store') }}" method="post">
        @csrf
        @if ($errors->any())
        ...
        <div>
          <div class="flex items-center justify-between gap-3">
            ...
            <a href="{{ route('owner.password.request') }}" class="text-sm font-extrabold text-[#2563EB]">نسيت كلمة المرور؟</a>
          </div>
        ...
        <button type="submit" class="inline-flex h-14 items-center justify-center rounded-2xl bg-[#2563EB] px-6 text-base font-extrabold text-white">تسجيل دخول</button>
      </form>
      <div class="mt-6 text-center">
        <p class="text-sm font-bold text-slate-600">لديك نشاط ولم تسجله بعد؟</p>
        <a href="{{ route('register') }}" class="mt-3 inline-flex w-full items-center justify-center rounded-2xl bg-[#2563EB] px-5 py-3 text-sm font-extrabold text-white shadow-sm">تسجيل نشاط جديد</a>
      </div>
    </section>
    <aside class="rounded-2xl border border-slate-200 bg-slate-950 p-6 text-white shadow-sm sm:p-8">
      <p class="text-sm font-extrabold text-blue-200">لوحة صاحب النشاط</p>
      <h2 class="mt-3 text-3xl font-extrabold leading-tight">كل أدوات إدارة نشاطك في مكان واحد</h2>
    </aside>
  </main>
</body>
</html>
